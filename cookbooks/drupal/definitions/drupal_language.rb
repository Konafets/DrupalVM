...
define :drupal_language, :action => :add, :name => nil, :dir => nil do
  ...
  when :import
    execute "drush_language_import #{params[:name]}" do
      cwd params[:dir]
      command "curl -o #{params[:dir]}/sites/default/files/drupal-#{node['drupal']['version']}.#{params[:name]}.po http://ftp.drupal.org/files/translations/7.x/drupal/drupal-#{node['drupal']['version']}.#{params[:name]}.po"
      command "#{node['drupal']['drush']['dir']}/drush -y language-import #{params[:name]} #{params[:dir]}/sites/default/files/drupal-#{node['drupal']['version']}.#{params[:name]}.po}"
    end
  else
